---
title: "Relatório do trabalho 1 de Data Mining 1 - Análise dos alugueres de bicicletas da Capital Bikeshare"
author: "Miguel Barbosa, Raul Ferreira, Tiago Martins"
date: "31 de Dezembro de 2017"
output:
  html_document:
    df_print: paged
---


```{r setup, include=FALSE}
source("importationAndCleanup.R")
knitr::opts_chunk$set(echo = TRUE)
```

## Introdução do problema

O data set disponibilizado para a realização deste trabalho contêm informação sobre alugueres de bicicletas da Capital Bikeshare, de Washington D.C.. O *data set* original tem um total de 731 observações e 16 variáveis. O conjunto dos dados está relacionado com um registo histórico dos anos 2011 e 2012. Para cada observação, ou seja, para cada dia de cada um dos dois anos, também existem as seguintes características: o *index* da observação, a data, a estação do ano, o ano, o mês do ano (1-12), a hora do dia (0-23), se o dia é um feriado ou não, qual o dia da semana, se é fim-de-semana ou não, (FALTA weathersit), temperatura normalizada, temperatura sentida normalizada, humidade normalizada, velocidade do vento normalizada, número de alugueres por utilizadores casuais, número de alugueres por utilizadores registados, e por fim, o total de bicicletas alugadas.


## Importação/Pré-processamentos dos dados

<!-- VER O PROBLEMA DAS SEASONS -->

Primeiramente, foi utilizada a função **read.csv** de forma a importar o *data set*, com o argumento **header** para indicar que o ficheiro contém os nomes das variáveis na sua primeira linha. Não foi utilizado o argumento **na.strings**, pois após uma verificação inicial dos dados, concluiu-se que não existiam nenhuns valores desconhecidos.

```{r import, eval=FALSE}
bikeSharing <- read.csv("Dataset/day.csv", header = TRUE)
```


De seguida, foram efetuados alguns testes para verificar se os valores de certas variáveis estavam corretos. Também foram criadas várias colunas novas e os valores de certas variáveis foram modificados, com o objetivo de tornar a interpretação dos dados mais fácil. 

```{r test1, eval=FALSE}
subset(bikeSharing, bikeSharing$casual + bikeSharing$registered != bikeSharing$cnt)
```

Com esse código verificou-se que para todas as observações, o valor da variável **cnt** corresponde de facto, à soma das variáveis **casual** e **registered**.

Na descrição dos dados do *data set*, era mencionado que na coluna **season**, o valor 1 representava a estação *spring*, o 2 a estação *summer*, o 3 representava a estação *fall*, e finalmente, o valor 4 o *winter*. No entanto averiguou-se que esses valores estavam errados. O valor 1 representa, na realidade, a estação *winter*, o 2 a estação *string*, *etc*. Procedeu-se assim, à correção da coluna **season**.

```{r test2, eval=FALSE}
change_season <- function(x) {
  sapply(x, function(x) {
    switch(as.character(x), "1" = "Winter", "2" = "Spring", "3" = "Summer", "4" = "Fall")
  })
}

bikeSharing$season <- change_season(bikeSharing$season)
bikeSharing$season <- factor(bikeSharing$season, 
                             levels = c("Spring", "Summer", "Fall", "Winter"), ordered = TRUE)
```

Foi realizada uma modificação na coluna **yr**, passando os valores 0 e 1 para 2011 e 2012, respetivamente.

```{r test3, eval=FALSE}
change_year <- function(x) {
  sapply(x, function(x) {
    switch(as.character(x), "0" = "2011", "1" = "2012")
  })
}

bikeSharing$yr <- change_year(bikeSharing$yr)
bikeSharing$yr <- factor(bikeSharing$yr, levels = c(2011, 2012), ordered = TRUE)
```

Da mesma forma, a coluna **weathersit** também foi modificada. Os valores 1, 2, 3 e 4 deram lugar a *Clear*, *Mist*, *Light rain/snow* e *Heavy rain/snow*, respetivamente.

```{r test4, eval=FALSE}
change_workd <- function(x) {
  sapply(x, function(x) {
    switch(as.character(x), "1" = "Clear", "2" = "Mist", "3" = "Light rain/snow", "4" = "Heavy rain/snow")
  })
}

bikeSharing$weathersit <- change_workd(bikeSharing$weathersit)
bikeSharing$weathersit <- factor(bikeSharing$weathersit, levels = c("Clear", "Mist", "Light rain/snow", "Heavy rain/snow"),
    ordered = TRUE
  )
```

A variável **mnth** foi alterada, para apresentar o nome do mês em vez de um inteiro de 1 a 12. A coluna **weekday** passou a ser representada pelo o nome do dia da semana, e não por um inteiro de 0 a 6. Finalmente, as variáveis **holiday** e **workingday** passaram a ter valores *booleanos* em vez do valor 0 ou 1.

```{r test5, eval=FALSE}
bikeSharing$mnth <- month(bikeSharing$mnth, label = TRUE, abbr = FALSE)

bikeSharing$weekday <- wday(bikeSharing$dteday, label = TRUE, abbr = FALSE)

bikeSharing$holiday <- as.logical(bikeSharing$holiday)

bikeSharing$workingday <- as.logical(bikeSharing$workingday)
```


Foram criadas 4 novas variáveis, que representam os valores reais das colunas **temp**, **atemp**, **hum** e **windspeed**.

```{r, eval=FALSE}
bikeSharing <- mutate(bikeSharing, real_temp = bikeSharing$temp * 41)

bikeSharing <- mutate(bikeSharing, real_atemp = bikeSharing$atemp * 41)

bikeSharing <- mutate(bikeSharing, real_hum = bikeSharing$hum * 100)

bikeSharing <- mutate(bikeSharing, real_windspeed = bikeSharing$windspeed * 67)
```


## Análise exploratória dos dados

```{r setup2, include=FALSE}
library(ggplot2)
library(dplyr)
library(grid)
library(tidyr)
library(scales)
```

```{r just_testing, echo=FALSE}
gr_by_month <- select(bikeSharing, yr, mnth, cnt) %>% group_by(yr, mnth) %>%
  summarise(total_cnt = sum(cnt))

mean_temp <- bikeSharing %>% group_by(mnth) %>% 
  summarise(mean_temperature = mean(real_temp))


gp1 <- gr_by_month %>% ggplot() + geom_bar(aes(x = mnth, y = total_cnt * 35 / 220000, fill = yr), stat = "identity", position = "dodge") +
  geom_line(data = mean_temp, aes(x = mnth, y = mean_temperature, group = 1), show.legend = FALSE) +
  scale_y_continuous(name = expression("Temperatura ("~degree~"C)"),
                     sec.axis = sec_axis(~ . * 220000 / 35, name = "Nº de alugueres"),
                     limits = c(0, 35)
                     ) +
  labs(x = "Mês")

gp1
```


```{r}
gr_by_weekd <-
  select(bikeSharing, weekday, casual, registered) %>% group_by(weekday) %>%
  summarise(casuals = sum(casual),
            registered = sum(registered))

#converter casual/registered em colunas tipo/contagem
gr_by_weekd2 <- gather(gr_by_weekd, type, count, casuals:registered)


#gráfico weekday vs tipo/contagem
#casuals alugam mais ao fim de semana, registered alugam mais durante a semana útil (talvez ate como meio de transporte para o trabalho)
gp3 <- ggplot(gr_by_weekd2, aes(weekday, count, fill = type)) +
  geom_bar(stat = "identity", position = "dodge") +
  scale_y_continuous(labels = comma)

gp3

```



## Definição de Modelo de previsão


