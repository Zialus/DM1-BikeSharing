...
Neste gráfico está representada a distribuição dos alugueres casuais e registados pelas diferentes estações do ano e pelos diferentes climas.

Como se pode ver neste conjunto de gráficos, a importância do clima é preponderante. Apesar de ser claro que existem diferenças no número de alugueres entre as 4 estações, os alugueres são feitos mais de acordo com o tipo de clima do que com a estação do ano em que se encontra. No caso do clima ser de chuva ou neve, o número de alugueres, tanto casuais como registados, é muito baixo, em todas as estações. Nos outros dois tipos de clima, limpo e nevoeiro, apesar de terem valores diferentes entre si, apresentam um número de alugueres relativamente igual durante todas as estações.

<!-- É também de salientar que em casos de chuva forte praticamente não existem alugueres de utilizadores casuais. -->
...
```{r predmodel, eval=FALSE}
library(caret)
library(e1071)
source("importationAndCleanup.R")

#pre processamento para predictive task
...
predictions<-predict(model_svmr,testing)
mae<-mean(abs(testing$cnt-predictions2)) 
